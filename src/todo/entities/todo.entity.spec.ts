...
describe('Todo ORM Mapping', () => {
  ...
  beforeEach(async () => {
    columns = getMetadataArgsStorage().columns.filter(metadata => metadata.target === Todo);
  });

  it('Todo should be mapped with @Entity({ name: \'todos\' })', () => {
    const entityMetadata = getMetadataArgsStorage().tables.find(metadata => metadata.target === Todo);
  ...
  it('title should be mapped with @Column({ name: \'title\' })', () => {
    const titleColumn = columns.find(column => column.propertyName === 'title');
  ...
  it('completed should be mapped  with @Column({ name: \'completed\' })', () => {
    const completedColumn = columns.find(column => column.propertyName === 'completed');
